...
* Your adversary could use Celebrite's UFED or other technical means to hack your phone. Such an adversary could be a customs officer at the border or a member of the security services. In this case, you should use [Graphene OS](https://grapheneos.org/) if it's possible. Using AntiForensic Tools with Graphene OS is a very questionable choice. Android AntiForensic Tools give you an opportunity to delete your secret data very fast by pressing power button several times in row, it's unclear whether it will be possible to detect that you have destroyed your data by thoroughly examining the device. Using root rights or other elevated privileges may serously compromise the security provided by Graphene OS. Perhaps you'd be better off using Graphene OS's built-in self-destruct option, but, unfortunately, wiping device data will be obvious to an adversary and that can make him angry.
