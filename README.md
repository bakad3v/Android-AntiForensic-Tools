...
* Clear & hide. The app will clear its data and hide itself. A less reliable alternative to self-destruct for use with dhizuku rights. Unstable. It has been noticed that it works better before the first unlocking of the device than after it. Requires Android 9 and above.
* Stop logd on start. The application will stop the logd service before the data deletion started. Thanks to this, actions related to data deletion will not be recorded in the logs.
* Stop logd on boot. The application will stop the logd service when the device boots up (more precisely, when the accessibility service is enabled). **Highly recommended to enable.**
### Removing data
...
Deleting files does not require special permissions. Also, it is not possible until the device is unlocked for the first time. If the trigger is activated before the device is unlocked for the first time, the deletion of files will begin only after unlocking, and only then can other actions, such as the self-destruct of the application, be started. **Therefore, deleting files is only recommended if you do not want or cannot grant special permissions to the application. Better keep your secret files in a separate profile!**
### Advanced settings
