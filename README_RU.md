...
Android AntiForensic Tools это бесплатное приложение с открытым исходным кодом, созданное, чтобы защитить ваши данные от могущественного злоумышленника, способного оказать на вас давление и заставить разблокировать устройство. Его цель - необратимо уничтожить ваши секретные данные в случае опасности, сделать это полностью незаметно для человеческого глаза и, насколько это возможно, сделать обнаружение удаления данных при анализе устройства более сложным (но не невозможным). Оно позволяет стереть данные устройства, профили пользователей или личные файлы при вводе фейкового пароля, подключении USB-устройства, многократном нажатии на кнопки или многократном вводе неправильного пароля. Приложение предлагает дополнительные опции для защиты вашего устройства от продвинутых злоумышленников: оно может удалить себя, запустить [TRIM](https://blog.elcomsoft.com/2025/06/what-trim-drat-and-dzat-really-mean-for-ssd-forensics/) после удаления данных, чтобы предотвратить восстановление удаленных данных, отключить логи, чтобы не оставить следов своих действий, запретить загрузку в безопасный режим и т.д.

Ваши данные защищены с помощью [Better Encrypted DataStore](https://github.com/bakad3v/BetterEncryptedDatastore).

## Модели угроз
В обсуждениях об этом приложении много раз возникало недопонимание моделей угроз, от которых оно может и не может защитить пользователя. Это приложение не может защищить вас от всех возможных угроз и на самом деле может **сделать вас более уязвимым к некоторым видам угроз**. Перед тем, как его использовать, подумайте, какой из сценариев для вас наиболее реалистичен:
* Ваш противник не собирается взламывать ваш телефон с помощью технических средств либо в силу своей технической неоснащённости, либо в силу спешки. Всё, что он будет использовать для того, чтобы получить пароль от устройства - это угрозы и физическая сила. Таким противником может быть коррумпированынй полицейский, пытающийся выбить пароль сразу при задержании, вор, абъюзивный партнёр в отношениях. В таком случае используйте Android AntiForensic Tools - вероятно, это лучшее приложение для защиты от угроз такого рода.
* Ваш противник может использовать Cellebrite UFED или другие технические средства для того, чтобы взломать ваш телефон и готов его тщательно анализировать. Таким противником может быть таможенник на границе или работник спецслужб. Практически единственный надёжный способ защититься от такой угрозы - установить [GrapheneOS](https://grapheneos.org/). К сожалению, Android AntiForensic Tools невосместим с GrapheneOS без рутирования. Вы можете установить Grephene OS с [рут-правами](https://github.com/schnatterer/rooted-graphene), и лучше использовать Android AntiFOrensicTools с GrapheneOS, чем с любой другой мобильной ОС, но рутирование GrapheneOS не рекомендуется, потому что получение root-прав увеличит площадь атаки и может ослабить защиту, предоставляемую GrapheneOS. Кроме того, хотя криминалисты скорее всего не смогут восстановить удалённые данные, они смогут выяснить, что вы их удаляли. Если для вас всё это проблема, не используйте Android AntiForensic Tools. Для защиты от такого рода угроз вам нужно использовать немодифицированную Graphene OS, если это возможно.
* Вы опасаетесь хаккерской атаки на ваш телефон, установки шпионского ПО вроде Pegasus или чего-то попроще. Скорее всего, для того, чтобы столкнуться с такой угрозой, вам нужно стать целью для преследования со стороны серьёзного противника. Не используйте Antiforensic Tools, оно сделает вас только более уязвимым к таким угрозам, используйте только Graphene OS.

## Возможности и ограничения
Перед тем, как использовать AntiForensic Tools, пожалуйста, обратите внимание на другое моё приложение, [0sakaCrypt](https://github.com/bakad3v/0sakaCrypt). В отличие от Android AntiForensic Tools, 0sakaCrypt не нужны root-права и приложение не должно уничтожать само себя, чтобы скрыть своё существование. 0sakaCrypt лучше AntiForensic Tools с точки зрения соблюдения принципа наименьших привелегий и сохранения модели безопасности Android. Кроме того, в отличие от AntiForensic Tools, приложение работает на немодифицированной Graphene OS. **0sakaCrypt в сочетании с Graphene OS рекомендуются для хранения чувствительных данных в большинстве случаев.**

На создание этого приложения меня вдохновило приложение [Wasted](https://github.com/x13a/Wasted) от x13a. Оно позволяет сбросить устройство до заводских настроек при вводе фейкового пароля, подключении USB или при иных триггерах. Wasted - большое достижение для антифоренсики на Android-устройствах, и в некоторых ситуациях оно может быть полезно, но у Wasted есть недостатки:                     
...
Моё приложение призвано исправить эти недостатки. Оно позволяет стереть все данные на новых версиях Android без root-прав с помощью Dhizuku, приложения для раздачи прав владельца устройства другим приложениям. Оно позволяет запретить загрузку в безопасном режиме с помощью Dhizuku, Shizuku или root. Но, что самое главное, оно позволяет удалять данные **относительно** незаметно. Вместо того, чтобы сбрасывать устройство, приложение позволяет удалить отдельный профиль пользователя или пользовательские файлы. После завершения всех операций приложение может удалить само себя.

К сожалению, у незаметности удаления данных есть ограничения, хотя удаление данных всё равно будет намного более незаметным, чем сброс устройства до заводских настроек. После удаления данных в разных местах системы (в логах, кэше, статистике и т.д.) остаётся множество свидетельств о том, что эти данные существовали. Стереть все эти следы [практически невозможно](https://discuss.grapheneos.org/d/40668-duress-persecuted-as-destruction-of-evidence-in-us/78), и продвинутый противник с полным доступом к устройству сможет их обнаружить. Приложение включает в себя некоторые дополнительные опции, которые позволят скрыть его существование на устройстве, следы ваших данных и роль приложения в стирании данных с Вашего устройства. Тем не менее, полностью скрыть следы удаления ваших данных с устройства невозможно.

Ещё один недостаток приложения заключается в том, что наиболее продвинутые функции приложения, в том числе и удаление приложением самого себя, требуют рут-прав для того, чтобы надёжно работать. Наличие рут-прав на устройстве [делает](https://madaidans-insecurities.github.io/android.html) его более уязвимым к некоторым типам атак. Вместо рут-прав можно использовать  [Dhizuku](https://github.com/iamr0s/Dhizuku) и [Shizuku](https://github.com/pixincreate/Shizuku/), однако функции самоуничтожения или запуска TRIM могут работать менее надёжно из-за ограничений, которыми обладают данные решения. Кроме того, эти решения тоже могут открывать дополнительные уязвимости. Ни root, ни Shizuku не работают с GrapheneOS, и единственный способ заставить работать это приложение на Graphene - это рутировать систему, что хоть и возможно, но может сделать её существенно менее безопасной и не рекомендуется разраюотчиками.

**Внимание: Dhizuku и Shizuku работают плохо по состоянию на октябрь 2025 года. Эта [версия](https://github.com/iamr0s/Dhizuku/releases/tag/v2.10.1) Dhizuku не может вызывать стирание данных устройства до API 35 (Android 15), эта [версия](https://github.com/iamr0s/Dhizuku/releases/tag/v2.11) Dhizuku может стирать данные устройства, но не работает в режиме BFU. Эта [версия](https://github.com/pixincreate/Shizuku/releases/tag/shizuku-v13.5.4.r1082.d2ed7207-release) Shizuku не работает с API 36 (Android 16), но может автоматически запускаться при загрузке устройства, эта [версия](https://github.com/RikkaApps/Shizuku/releases/tag/v13.6.0) Shizuku работает с API 36, но автозапуск у неё не работает. На данный момент функционал Dhizuku и Shizuku полностью работает только на API 35 (Android 15).**
...
**Если вы обновляетесь до версии v2.0.3 или более поздней с более ранней версии приложения, права администратора могут быть отключены. Предоставьте их снова.**  
